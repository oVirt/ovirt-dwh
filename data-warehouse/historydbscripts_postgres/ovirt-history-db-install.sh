...
#
# internal script - run only by ovirt-dwh-setup.py after creating CA
#
# this script will create the ovirt_history db
...
oVirt_HISTORY_DB_SCRIPTS_DIR=/usr/share/ovirt-dwh/db-scripts
oVirt_HISTORY_DB_CREATE_SCRIPT=create_db.sh
...
initLogFile()
{
    ...
    echo "#ovirt history db installer log file on $HOST" > $LOG_FILE
}
...
createDB()
{
    ...
    if [[ -d "$oVirt_HISTORY_DB_SCRIPTS_DIR" && -e "$oVirt_HISTORY_DB_SCRIPTS_DIR/$oVirt_HISTORY_DB_CREATE_SCRIPT" ]]
    then
        pushd $oVirt_HISTORY_DB_SCRIPTS_DIR >> $LOG_FILE
        #TODO: to we need to verify if the db was already created? (we can create a new file and check if exists..)
        $SHELL $oVirt_HISTORY_DB_CREATE_SCRIPT -u $DB_ADMIN >> $LOG_FILE 2>&1
        _verifyRC $? "error, failed creating ovirt history db"
        popd >> $LOG_FILE

    else
        echo "[$SCRIPT_NAME] error, can't find create_db script at $oVirt_HISTORY_DB_SCRIPTS_DIR/$oVirt_HISTORY_DB_CREATE_SCRIPT"
        exit 1
    fi
}
